...
func (o *GetV1DevicesDeviceURL) Build() (*url.URL, error) {
	...
	var _path = "/v1/devices/{device}"

	device := o.Device
	if device != "" {
		_path = strings.Replace(_path, "{device}", device, -1)
	} else {
		return nil, errors.New("device is required on GetV1DevicesDeviceURL")
	}
...
func (o *GetV1DevicesDeviceURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetV1DevicesDeviceURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetV1DevicesDeviceURL")
	}
